...
function detectScrollControlledMedia(innerBlocks) {
	...
	while (stack.length) {
		...
		if (
			...
			hasAnimatedGif = candidateUrls.some((url) => /\.gif(?:[?#].*)?$/i.test(String(url)));
		}
	...
	return { hasVideo, hasAnimatedGif };
}
...
registerBlockType(metadata.name, {
	...
	edit: ({ attributes, setAttributes, clientId }) => {
		const {
			...
			mediaScrollPlaybackDirection,
			mediaScrollDirectionLimit,
			mediaScrollPlaybackCycles,
			mediaScrollProgressSource,
			mediaScrollViewportEdge,
			mediaScrollViewportStart,
			mediaScrollViewportEnd,
			mediaScrollStartAtPageTop,
			mediaScrollDocumentStart,
		...
		const detectedScrollMedia = useMemo(() => detectScrollControlledMedia(innerBlocks), [innerBlocks]);
		...
		useEffect(() => {
			...
			if (!['forward', 'backward'].includes(mediaScrollPlaybackDirection)) {
				updates.mediaScrollPlaybackDirection = 'forward';
				shouldUpdate = true;
			}

			if (!['both', 'down', 'up'].includes(mediaScrollDirectionLimit)) {
				updates.mediaScrollDirectionLimit = 'both';
				shouldUpdate = true;
			}

			if (!['viewport', 'document', 'parent'].includes(mediaScrollProgressSource)) {
				updates.mediaScrollProgressSource = 'viewport';
				shouldUpdate = true;
			}

			if (!['top', 'bottom'].includes(mediaScrollViewportEdge)) {
						...
						{isMediaScroll && (
							<>
								<SelectControl
									...
									onChange={(value) => setAttributes({ mediaScrollPlaybackDirection: value })}
								/>
								<SelectControl
									...
									onChange={(value) => setAttributes({ mediaScrollDirectionLimit: value })}
								/>
								<RangeControl
									label={__('Number of times to play', 'anilibrary')}
									value={mediaScrollPlaybackCycles}
									onChange={(value) => setAttributes({ mediaScrollPlaybackCycles: Math.max(1, Number(value) || 1) })}
									min={1}
									max={10}
									step={1}
								...
								<SelectControl
									...
									onChange={(value) => setAttributes({ mediaScrollProgressSource: value })}
								/>
								{mediaScrollProgressSource === 'viewport' && (
									<>
										<SelectControl
											...
											onChange={(value) => setAttributes({ mediaScrollViewportEdge: value })}
										/>
										<RangeControl
											label={__('Start when that part is this far down the screen (%)', 'anilibrary')}
											value={mediaScrollViewportStart}
											onChange={(value) => setAttributes({ mediaScrollViewportStart: Number(value) || 0 })}
											min={0}
											max={100}
											step={1}
										...
										<RangeControl
											...
											onChange={(value) => setAttributes({ mediaScrollViewportEnd: Number(value) || 0 })}
											min={0}
											max={100}
											step={1}
										...
										<ToggleControl
											...
											onChange={(value) => setAttributes({ mediaScrollStartAtPageTop: value })}
									...
									<>
										...
										{!mediaScrollStartAtPageTop && (
											<RangeControl
												...
												onChange={(value) => setAttributes({ mediaScrollDocumentStart: Number(value) || 0 })}
												min={0}
												max={100}
												step={1}
											/>
										...
										<RangeControl
											...
											onChange={(value) => setAttributes({ mediaScrollDocumentEnd: Number(value) || 0 })}
											min={0}
											max={100}
											step={1}
										/>
